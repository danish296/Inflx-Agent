...
def mock_lead_capture(name: str, email: str, platform: str) -> str:
    ...
    IMPORTANT: Only invoke this once `name`, `email`, AND `platform` are all
    confirmed from the user. Never call it with placeholders or missing data.
...
2. High-intent signals (user wants to sign up / try / buy a plan):
   - You must collect three fields: NAME, EMAIL, CREATOR PLATFORM.
   - Ask for only the MISSING fields, concisely, one or two at a time.
   - Do NOT call `mock_lead_capture` until all three are confirmed.
   - Once all three are collected, CALL `mock_lead_capture(name, email, platform)`
     exactly once, then confirm success to the user.

3. Casual greetings -> reply warmly in one short line and invite the user to ask
   about plans or features.

4. Keep responses concise (1-4 sentences). No markdown headings, no emojis.
...
def agent_node(state: AgentState) -> dict:
    ...
    # Detect whether the model just called `mock_lead_capture` so downstream
    # turns can reflect that the lead is now captured.
    updated: dict = {"messages": [response]}
    for call in getattr(response, "tool_calls", []) or []:
        if call.get("name") == "mock_lead_capture":
